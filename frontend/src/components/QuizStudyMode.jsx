...
export default function QuizStudyMode({ deck, onBack, onUpdateCardStatus }) {
    ...
    useEffect(() => {
        ...
        if (alreadyAnswered) {
            ...
            if (savedOption) {
                // HYPER-FUZZY MATCH: Remove all punctuation, symbols, and extra whitespace
                const normalize = (str) => String(str).toLowerCase().replace(/[^a-z0-9]/g, '');
                const normalizedSaved = normalize(savedOption);
                
                const savedOptionIndex = options.findIndex(opt => {
                    const normalizedOpt = normalize(opt);
                    return normalizedOpt === normalizedSaved;
                });
                
                if (savedOptionIndex !== -1) {
                    setUserSelectedIdx(savedOptionIndex);
                    setIsConfirmed(true);
                } else {
        ...
        setShowHint(false);
    }, [currentIndex, card.id, options, deck.quizType, deck.certLevel]);
