...
import { getVaultStats, getDomainSnapshots, resolveCardDomains, loadVaultFromStorage } from '../utils/storage';
...
const DomainGrid = ({ decks, certLevel, onSelectDomain, studyMode, quizType, selectedDomain, onResetDomain }) => {
  ...
  const domainProgress = useMemo(() => {
    const progress = {
      'People': { mastered: 0, total: 0, attempted: 0, sumScores: 0 },
      'Organization': { mastered: 0, total: 0, attempted: 0, sumScores: 0 },
      'Workplace': { mastered: 0, total: 0, attempted: 0, sumScores: 0 },
      'Competencies': { mastered: 0, total: 0, attempted: 0, sumScores: 0 }
    };

    const activeVault = loadVaultFromStorage();

    decks.forEach(deck => {
      deck.cards.forEach(card => {
        const targetDomains = resolveCardDomains(card, certLevel, deck.title, activeVault);
        ...
        targetDomains.forEach(domain => {
          ...
          const val = card[statusKey];
          if (val === 'mastered' || val === 'correct' || val === 'incorrect' || (val && val.toString().startsWith('difficulty-'))) {
            progress[domain].attempted++;
            if (val && val.toString().startsWith('difficulty-')) {
               progress[domain].sumScores += (parseInt(val.split('-')[1]) || 0);
            }
          }
          if (val === 'mastered' || val === 'correct' || val === 'difficulty-5' || val === 'difficulty-4') {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
      {DOMAINS.map(domain => {
        ...
        const prog = domainProgress[domain.id] || { mastered: 0, total: 0, attempted: 0 };
        const percentProgress = prog.attempted > 0 ? Math.round((prog.mastered / prog.attempted) * 100) : 0;
        const readyCount = quizType === 'intelligent' ? stats.intelligent : stats.simple;
        const isReady = stats.total > 0 && readyCount === stats.total;
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    const snapshotVal = studyMode === 'traditional' 
                        ? (prog.attempted > 0 ? ((prog.sumScores / prog.attempted) / 5) * 100 : 0) 
                        : percentProgress;
                    onResetDomain(domain.id, snapshotVal);
                  }}
                  style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', display: 'flex' }}
            ...
            <div style={{ zIndex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem', alignItems: 'flex-end' }}>
                {studyMode === 'traditional' ? (
                   <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: '600' }}>Confidence GPA ({prog.attempted > 0 ? (prog.sumScores / prog.attempted).toFixed(1) : "0.0"} / 5.0)</span>
                ) : (
                   <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: '600' }}>Current Proficiency ({prog.mastered}/{prog.attempted})</span>
                )}
                {studyMode === 'traditional' ? (
                   <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'white' }}>{prog.attempted > 0 ? (prog.sumScores / prog.attempted).toFixed(1) : '-'}</span>
                ) : (
                   <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: 'white' }}>{prog.attempted > 0 ? `${percentProgress}%` : '-'}</span>
                )}
              </div>
              <div style={{ width: '100%', height: '5px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ 
                  width: prog.attempted > 0 ? `${studyMode === 'traditional' ? ((prog.sumScores / prog.attempted) / 5) * 100 : percentProgress}%` : '0%', 
                  height: '100%', background: domain.gradient, boxShadow: `0 0 8px ${domain.color}44` 
                }} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.8rem', paddingTop: '0.4rem', borderTop: '1px solid rgba(255,255,255,0.03)', fontSize: '0.65rem', color: 'var(--text-muted)', zIndex: 1, fontWeight: 'bold' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                ...
                {readyCount} Questions Ready ({Math.max(0, readyCount - prog.attempted)} Remaining)
              </div>
              {isReady && (
