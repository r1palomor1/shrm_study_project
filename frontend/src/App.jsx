...
function App() {
  ...
  const handleStartTestFromSidebar = (isWeightedInput = false) => {
    const isWeighted = (isWeightedInput === true);
  ...
  const handleStartTestDirectly = async (config) => {
    ...
    // EXCELLENCE: Fetch current available pool before deciding whether to restore session
    const effectiveLength = lengthReq === -1 ? 9999 : lengthReq;
    const filter = { domainId, length: effectiveLength, isWeighted };
    const freshData = await getQuizDataByFilter(decks, filter, testType, level, (isAudio ? 'audio' : studyMode));

    if (!isWeighted) {
        const savedSessionStr = localStorage.getItem(activeSessionKey);
        if (savedSessionStr) {
            const savedSession = JSON.parse(savedSessionStr);
            console.log(`[DEBUG] Session Found in LocalStorage | Pool Size: ${savedSession.cards?.length || savedSession.cardIds?.length || 0} | Available Pool: ${freshData.totalAvailable} | Requested: ${lengthReq}`);

            // STRUCTURAL GUARD: Only restore if requested length matches AND the total pool hasn't grown since last save
            if (savedSession.requestedLength === lengthReq && (savedSession.totalOriginalCards === freshData.totalAvailable)) {
                ...
                if (isAudio) {
                    console.log('[AUDIO DEBUG] Session key:', activeSessionKey, '| Cards:', refreshedCards.length, '| Resume Index:', savedSession.initialIndex);
                ...
                // Stale or invalid session key - clear it
                console.info(`[DEBUG] Clearing STALE session due to pool growth or length mismatch.`);
                localStorage.removeItem(activeSessionKey);
            }
        }
    }

    // Fresh launch — no saved session OR session was purged due to pool update
    if (isAudio) console.log('[AUDIO DEBUG] Performing FRESH launch. Key:', activeSessionKey);
    
    if (freshData.cards.length === 0) {
      setConfirmModal({
        isOpen: true,
        type: 'warning',
    ...
    // PROGRESS RECOVERY: Start exactly at the FIRST untouched card in this mode
    const firstUnplayedIdx = freshData.cards.findIndex(c => {
      const status = c[statusKey];
      if (isAudio) return !c.status_audio_seen || c.status_audio_seen !== 'seen';
      // EXCELLENCE: Treat any score (difficulty-X), seen, or mastered as 'played'
    ...
    const storageOptimizedCards = isAudio ? 
        freshData.cards.map(c => ({ id: c.id })) : 
        freshData.cards;

    const activeDeckToSave = { 
      title: domainId === 'ALL' ? '' : `Domain: ${domainId}`, 
      cards: storageOptimizedCards, 
      totalOriginalCards: freshData.totalAvailable, 
      initialIndex: initialIndex, 
      quizType: testType, 
      certLevel: level,
      isUnderStrength: freshData.isUnderStrength,
    ...
    const vault = loadVaultFromStorage();
    const hydratedCards = freshData.cards.map(c => {
  ...
  return (
    <div className="app-container animate-fade-in">
      ...
      {isMatrixOpen && <div className="fixed-overlay animate-fade-in" onClick={() => setIsMatrixOpen(false)} style={{ zIndex: 10002 }}><div onClick={e => e.stopPropagation()} style={{ width: '90%', maxWidth: '1200px' }}><VaultHealthMatrix decks={decks} certLevel={certLevel} onSmartSync={handleBulkWarmUp} onSyncTopic={handleSurgicalTopicSync} onClose={() => setIsMatrixOpen(false)} isSyncing={isWarmingUp} syncProgress={warmUpProgress} syncStatus={warmUpStatus} /></div></div>}
